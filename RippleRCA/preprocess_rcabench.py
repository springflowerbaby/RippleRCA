"""
RCAbench -> LADS-Causal 预处理脚本

目标：
- 将 RCAbench/scenarios/<scenario>/ 下的 normal/abnormal parquet + env.json + injection.json
  转为一个“轻量、稳定、可复用”的预处理目录，供 DatasetRCAbench 直接读取并构图。

输出目录结构（默认：<output_root>/rcabench_preprocessed）：
  rcabench_preprocessed/
    scenarios/
      <scenario_name>/
        meta.json
        nodes.csv
...
说明：
- 本脚本依赖 pandas 读取 parquet，需要安装 pyarrow 或 fastparquet。
- 本脚本不强依赖具体 parquet schema，会做列名探测；若无法识别会给出可操作的报错信息。
"""
...
def canonicalize_service_name(name: Optional[str]) -> Optional[str]:
    """
    将 service/container/span 等字段统一成“节点服务名”风格，用于 groundtruth / traces / metrics 的对齐。

    目标：
    - 去掉协议头、端口、路径、query
    - 去掉 k8s 域名后缀（.svc / .cluster.local 等）
    - 去掉引号/空白，统一小写
    """
    if name is None:
        return None
...
def _parse_service_from_span(span_name: str) -> Optional[str]:
    ...
    if not s:
        return None
    # 简单从 "http://" 或 "https://" 之后截取 host，再去掉端口和路径
    lower = s.lower()
    for prefix in ("http://", "https://"):
        if prefix in lower:
            try:
...
def _infer_service_from_any(row: pd.Series) -> Optional[str]:
    for key in ("service", "service_name", "cmdb_id", "container", "container_name", "pod", "pod_name"):
...
def _coerce_epoch_seconds(ts) -> Optional[int]:
    """
    将各种时间戳格式统一为 epoch seconds(int)。

    RCAbench 的 metrics/traces 可能出现：
    - datetime / pandas.Timestamp（例如 2025-08-18 23:02:44.56+00:00）
    - ISO 字符串
    - 数字时间戳（ns/us/ms/s）
    """
    # 1) 先尝试解析 datetime / timestamp / ISO 字符串
    try:
        t = pd.to_datetime(ts, errors="coerce", utc=True)
        if pd.notna(t):
            # pandas Timestamp -> python datetime -> epoch seconds
    ...
    if v is None:
        return None
    # RCAbench traces/logs 可能是 ns/us/ms；这里做一个简单数量级判断
    if v > 10**15:  # ns
...
def _map_metric_to_feature(metric_name: str) -> Optional[str]:
    ...
    # Round 2（中风险观察）：只做 workload/error 的“可回滚补丁”。
    # 注意：不要同步改 _map_metric_to_feature_reference，便于 mapping_debug 显示差分。
    if m in {
        "k8s.pod.phase",
        "k8s.container.ready",
        "k8s.namespace.phase",
    ...
    if any(k in m for k in ("disk", "blkio", "fs_read", "fs_write", "io")):
        return "diskio"
    if any(k in m for k in ("net_in", "receive_bytes", "rx_bytes", "network_receive", "recv")):
        return "net_in"
    if any(k in m for k in ("net_out", "transmit_bytes", "tx_bytes", "network_transmit", "send")):
        return "net_out"
    if any(k in m for k in ("qps", "rps", "throughput", "request_total", "requests", "workload")):
        return "workload"
...
def _map_metric_to_feature_reference(metric_name: str) -> Optional[str]:
    """
    参考映射（冻结基线）：用于做“当前规则 vs 参考规则”的差分统计。
    默认与当前规则一致；后续若调整 _map_metric_to_feature，可保留此函数不变用于对比。
    """
    m = str(metric_name).lower()
    if any(k in m for k in ("cpu", "container_cpu", "process_cpu")):
        return "cpu"
...
def _build_mapping_debug(
    ...
    """
    统计场景级映射调试信息（按 rows + 按 unique metrics）：
    - unmapped_hits
    - mapped_migrations（reference -> current）
    - bucket_delta（current - reference）
    """
    metrics = pd.concat([normal_long["metric"], abnormal_long["metric"]], axis=0).dropna().astype(str)
    if metrics.empty:
        return {
            "scenario": scenario,
            "rows_total": 0,
    ...
    def _pack(sub: pd.DataFrame) -> Dict[str, Any]:
        ...
        unmapped_metrics = (
            sub[sub["bucket_cur"].isna()]["metric"].value_counts().head(top_unmapped_n).to_dict()
        )
    ...
    # unique metrics 统计（按 metric 名去重）
...
def _load_metrics_long(parquet_path: Path) -> pd.DataFrame:
    """
    读取 RCAbench metrics parquet，并转换为统一 long schema：
      ts(int seconds), service(str), metric(str), value(float)
    """
    ...
    service_col = _detect_col(df.columns, ("service", "service_name", "container", "container_name", "pod", "pod_name"))

    if ts_col and metric_col and value_col:
        out = df[[ts_col, metric_col, value_col] + ([service_col] if service_col else [])].copy()
        out.rename(columns={ts_col: "ts_raw", metric_col: "metric", value_col: "value"}, inplace=True)
        if service_col:
        ...
        # service 缺失时：优先从 metric 名称解析（否则很多数据会全部落到 unknown，导致只有 1-2 个节点）
        if out["service"].isna().any() or (out["service"].astype("string") == "unknown").all():
            parsed = out["metric"].map(_parse_service_from_metric_name)
            out["service"] = out["service"].fillna(parsed)
            # 仍缺失时，再尝试从其他字段推断（若存在）
            if out["service"].isna().any():
                for alt in ("service", "service_name", "container", "container_name", "pod", "pod_name"):
    ...
    # 如果 parquet 是宽表：尝试找时间列，然后 melt
    if ts_col:
        wide = df.copy()
        wide.rename(columns={ts_col: "ts_raw"}, inplace=True)
        id_vars = ["ts_raw"]
        value_vars = [c for c in wide.columns if c not in id_vars]
        melted = wide.melt(id_vars=id_vars, value_vars=value_vars, var_name="metric", value_name="value")
        ...
        # 宽表列名里通常包含 service 前缀：直接解析
        melted["service"] = melted["metric"].map(lambda x: _parse_service_from_metric_name(x) or "unknown")
...
def _compute_service_timeseries(
    ...
    """
    输出：
      service_timeseries.csv 的 DataFrame（columns: ts + <service>::<feature>）
    """
    n0, n1 = normal_range
    a0, a1 = abnormal_range
    # 统一时间轴：只用 abnormal 窗口（LADS 实际 RCA 用异常时段）
    ts_index = list(range(a0 - (a0 % freq_s), a1 + 1, freq_s))

    # 先算 normal 统计：按 (service, feature) -> mean/std
    ...
    # 确保 groundtruth service 被纳入（即使该服务指标缺失，也要有节点）
    ...
    # abnormal resample -> mean per bucket per ts_bin
    ...
    # 透视成宽表：每个 service 的每个 feature 一列
    cols = []
    for s in services:
    ...
    pivot = agg.pivot_table(index="ts", columns=["service", "feature"], values="z", aggfunc="mean")
    # 填充缺失
    for s in services:
        for fk in FEATURE_KEYS:
...
def _compute_service_delta_metrics(
    ...
    """
    计算每个 service、每个 feature bucket 的 per-scenario delta metrics。

    设计目标：
    - 用“相对变化”替代绝对值，消除不同服务/指标的静态尺度差异
    - 输出保持与旧版兼容：仍提供 delta（单列），供下游直接读取 8 维

    这里把 delta 定义为稳健的 robust-z-delta（以 normal 的 MAD 作为尺度）：
      delta = (abnormal_mean - normal_mean) / (normal_MAD + eps)

    同时额外输出一些辅助列（diff / z / log_ratio），便于后续消融，但不强制下游使用。

    返回列：
      service, feature, delta, diff, z, robust_z, log_ratio
    """
    n0, n1 = normal_range
    a0, a1 = abnormal_range
    ...
    if normal_b.empty and abnormal_b.empty:
        return pd.DataFrame(columns=["service", "feature", "delta", "diff", "z", "robust_z", "log_ratio"])

    def _mad(x: pd.Series) -> float:
        # median absolute deviation（未做 1.4826 标准化；这里只作为稳健尺度）
        med = float(x.median()) if len(x) else 0.0
        return float((x - med).abs().median()) if len(x) else 0.0

    n_agg = (
        normal_b.groupby(["service", "feature"])["value"]
        .agg(normal_mean="mean", normal_std="std", normal_median="median", normal_mad=_mad)
        .reset_index()
    )
    a_agg = (
    ...
    # 更稳健的 ratio：用 mean 计算 log-ratio，并避免 0/负数（取 abs 并加 eps）
    ...
    # 兼容列：delta 使用 robust_z，并做剪裁避免极端值主导
...
def _compute_trace_service_scores(conclusion_parquet: Path) -> pd.DataFrame:
    """
    从 conclusion.parquet 计算每个 service 的 trace 异常强度：
      - latency_ratio: AbnormalAvgDuration / NormalAvgDuration 的最大值
      - succ_drop: NormalSuccRate - AbnormalSuccRate 的最大值
    """
    try:
    ...
    if "AbnormalAvgDuration" in df.columns and "NormalAvgDuration" in df.columns:
        df["latency_ratio"] = df["AbnormalAvgDuration"] / (df["NormalAvgDuration"] + eps)
    else:
    ...
    agg = (
        df.groupby("service")[["latency_ratio", "succ_drop"]]
        .max()
        .reset_index()
    )
    return agg
...
def _build_edges_from_traces(trace_parquet: Path, services: List[str]) -> List[Tuple[str, str]]:
    """
    从 traces parquet 尝试构造 service->service 边。
    如果 schema 不可识别，返回空边集合（DatasetRCAbench 会退化为自环图）。
    """
    try:
    ...
    # 尝试识别 caller/callee
    caller_col = _detect_col(df.columns, ("caller_service", "src_service", "source_service", "parent_service", "caller"))
    callee_col = _detect_col(df.columns, ("callee_service", "dst_service", "target_service", "child_service", "callee"))
    if not caller_col or not callee_col:
        # 退化：尝试 span_name 里提取 "http://<service>" 之类（不保证成功）
...
def _extract_services_and_edges_from_traces(trace_parquet: Path) -> Tuple[List[str], List[Tuple[str, str]]]:
    """
    从 traces parquet 尝试提取：
    - 全部出现过的服务集合（caller/callee）
    - 调用边 caller->callee

    说明：
    RCAbench 的 traces schema 可能不同，这里用“列名探测 + 兜底”方式：
    - 优先找 caller/callee service 列
    - 若找不到，返回空（由上层回退到 metrics+gt_service）
    """
    try:
        df = pd.read_parquet(trace_parquet)
    except Exception:
    ...
    caller_col = _detect_col(
        df.columns,
        ("caller_service", "src_service", "source_service", "parent_service", "caller", "upstream_service"),
    )
    callee_col = _detect_col(
        df.columns,
        ("callee_service", "dst_service", "target_service", "child_service", "callee", "downstream_service"),
    )
    ...
    # === RCAbench 常见 traces schema ===
    # columns: trace_id, span_id, parent_span_id, service_name, attr.k8s.service.name ...
    trace_id_col = _detect_col(df.columns, ("trace_id", "traceId"))
    span_id_col = _detect_col(df.columns, ("span_id", "spanId", "id"))
    parent_span_id_col = _detect_col(df.columns, ("parent_span_id", "parentSpanId", "parent_id"))
    service_col = _detect_col(df.columns, ("service_name", "serviceName", "attr.k8s.service.name"))
    if not service_col:
    ...
    if trace_id_col and span_id_col and parent_span_id_col and service_col:
        sub = df[[trace_id_col, span_id_col, parent_span_id_col, service_col]].dropna(subset=[trace_id_col, span_id_col, service_col]).copy()
        sub = sub.rename(
            columns={
        ...
        # parent_span_id -> span_id 映射：在每个 trace 内找父 span 的 service
        # 自连接：child.parent_span_id == parent.span_id && same trace_id
        parents = sub[["trace_id", "span_id", "service"]].rename(columns={"span_id": "parent_span_id", "service": "parent_service"})
...
def preprocess_one_scenario(
    ...
    # groundtruth 可能是多个服务（例如 network bandwidth: service + mysql）
    try:
    ...
    # traces：优先用 abnormal_traces 构造“全服务集合 + 调用边”
    ...
    if abnormal_traces_pq.exists():
        services_from_traces, edges_from_traces = _extract_services_and_edges_from_traces(abnormal_traces_pq)
    # 若 abnormal 为空，尝试 normal
    if not services_from_traces:
        normal_traces_pq = scenario_dir / "normal_traces.parquet"
        if normal_traces_pq.exists():
            services_from_traces, edges_from_traces = _extract_services_and_edges_from_traces(normal_traces_pq)
    ...
    # 强制把所有 groundtruth services 纳入节点集合（即使 metrics/trace 缺失，也必须有节点可命中）
    if gt_services:
        for gts in gt_services:
            if gts and gts not in services:
                services = [gts] + services
        services = services[:200]
        # 同步扩列（缺失服务填 0），避免 nodes.csv 有但 timeseries 没有
    ...
    # 用 traces 扩展服务集合（让评估有意义：每个场景不再只有 1-2 个节点）
    if services_from_traces:
        ...
        for s in services_from_traces:
            if s and s not in merged:
                merged.append(s)
        # 再把 metrics 中出现的 service 也加进来（避免 traces 漏）
        for s in services:
            if s and s not in merged:
                merged.append(s)
        services = merged[:200]

        # 重新把 service_timeseries 扩列：缺失服务全 0（保持 CSV 结构一致）
        # 当前 service_ts 可能只包含少量 metrics service 列
        if not service_ts.empty:
            ts_col = service_ts["ts"]
    ...
    # === per-scenario delta metrics：基于 normal vs abnormal 的均值差 ===
    ...
    # 透视为每个 service 一行、8 维 FEATURE_KEYS
    if not delta_df.empty:
        delta_pivot = (
            delta_df.pivot_table(index="service", columns="feature", values="delta", aggfunc="mean")
            .reset_index()
        )
    else:
        delta_pivot = pd.DataFrame(columns=["service"] + list(FEATURE_KEYS))

    # 统一到当前 services 列表，缺失补 0
    rows = []
    for s in services:
        row = {"service": s}
    ...
    # 从 scenario 名称提取可读故障类型（用于评估分桶）
    failure_type_str = infer_rcabench_failure_type(scenario_dir.name, inj)

    # edges：优先使用 traces 直接提取的边；否则回退到旧逻辑（可能为空）
    edges: List[Tuple[str, str]] = []
    if edges_from_traces:
        edges = [(u, v) for (u, v) in edges_from_traces if u in services and v in services]
    else:
        if abnormal_traces_pq.exists():
    ...
    # 保存 canonicalized 的 groundtruth service 列表，供下游 dataset 直接使用
    try:
        if "ground_truth" not in meta or meta["ground_truth"] is None:
            meta["ground_truth"] = {}
        if isinstance(meta["ground_truth"], dict):
            meta["ground_truth"]["service"] = gt_services if gt_services else (meta["ground_truth"].get("service") or [])
    except Exception:
        pass
    ...
    if conclusion_pq.exists():
        ...
        for s in services:
            ...
            rows_ts.append(row)
        trace_out = pd.DataFrame(rows_ts, columns=["service", "latency_ratio", "succ_drop"])
...
def _aggregate_mapping_debug(all_debug: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    unmapped_rows_ref = int(sum(d["rows"]["unmapped_hits"]["reference"] for d in all_debug))
    unmapped_rows_cur = int(sum(d["rows"]["unmapped_hits"]["current"] for d in all_debug))
    unmapped_uniq_ref = int(sum(d["unique_metrics"]["unmapped_hits"]["reference"] for d in all_debug))
    unmapped_uniq_cur = int(sum(d["unique_metrics"]["unmapped_hits"]["current"] for d in all_debug))
    ...
    return {
        "summary": {
            ...
            "rows_unmapped_delta_current_minus_reference": int(unmapped_rows_cur - unmapped_rows_ref),
            ...
            "unique_unmapped_delta_current_minus_reference": int(unmapped_uniq_cur - unmapped_uniq_ref),
        },
        "scenarios": all_debug,
    }
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Preprocess RCAbench for RippleRCA")
    p.add_argument("--rcabench_root", type=str, required=True, help="RCAbench 根目录（包含 scenarios/）")
    p.add_argument(
        "--output_root",
        type=str,
        default=None,
    ...
    p.add_argument("--max_scenarios", type=int, default=10_000, help="最多处理多少个 scenario（调试用）")
    p.add_argument(
        "--skip_existing",
        action="store_true",
...
def main() -> None:
    ...
    if not scenarios_root.exists():
        direct_scenarios = [
            ...
            if p.is_dir() and (p / "injection.json").exists() and (p / "env.json").exists()
        ] if rcabench_root.exists() else []
        if direct_scenarios:
    ...
    output_root = Path(args.output_root) if args.output_root else (rcabench_root / "rcabench_preprocessed")
    ...
    for sdir in scenario_dirs:
        ...
        if bool(args.skip_existing) and _preprocessed_scenario_complete(scenario_out_dir):
            skipped += 1
            ok += 1
            continue
        try:
    ...
    if args.mapping_debug_path:
        ...
        md_out.write_text(
            json.dumps(_aggregate_mapping_debug(all_mapping_debug), ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
