...
class myDGLDataset(DGLDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.groundtruths: List[Tuple[str, int]] = []
    ...
    def label_to_groundtruth(
        self, label: Dict, node_df: pd.DataFrame, edge_df: pd.DataFrame
    ):
        groundtruth = set()
        if label["level"] == "service":
            service = label["cmdb_id"]
            pod_ids = node_df[
                (node_df["node_type"] == "pod")
                & (node_df["node_name"].str.startswith(service))
            ]["node_id"].tolist()
        elif label["level"] == "pod":
            service = self.pod_to_service(label["cmdb_id"])
            pod_ids = node_df[node_df["node_name"] == label["cmdb_id"]][
                "node_id"
            ].tolist()
        ...
        if label["failure_type"] in self.special_failure_types:
            ...
            for api_id in api_ids:
                tmp = edge_df[
                    (edge_df["edge_type"] == "api2api") & (edge_df["src"] == api_id)
                ]["dst"].tolist()
    ...
    def process(self):
        if self.use_split_info:
            with open(
                os.path.join(self.data_dir, "..", f"{self._name}_split.json"), "r"
            ) as f:
        ...
        for date in self.dates:
            fault_file_path = os.path.join(
                self.data_dir, "..", f"groundtruth_csv/groundtruth-k8s-1-{date}.csv"
            )
        ...
        for date in sorted(self.dates):
            ...
            for timestamp_at_minute in tqdm(
                range(start_timestamp, end_timestamp, 60), desc=f"Date: {date}"
            ):
                if timestamp_at_minute not in faults:
                    label = {
                        "timestamp": timestamp_at_minute,
                        "level": "",
                        "cmdb_id": "",
                        "failure_type": "",
                    }
                ...
                if (
                    self.use_split_info
                    and label["timestamp"] // 60 * 60 not in inject_timestamps
                ):
                    continue
                node_feats = {}
                edge_feats = {}
                ...
                for ntype in self.ntypes:
                    ...
                    time_str = datetime.fromtimestamp(timestamp_at_minute).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    ...
                    try:
                        ...
                        if len(df) > num_nodes:
                            ...
                            logger.warning(
                                f"Feature CSV has {len(df)} rows but graph has {num_nodes} nodes for {ntype}. Padding with zeros."
                            )
                            missing_rows = num_nodes - len(df)
                            padding_df = pd.DataFrame(
                                0, index=range(missing_rows), columns=df.columns
                            )
                            df = pd.concat([df, padding_df], ignore_index=True)
                        node_feats[ntype] = th.tensor(
                            df[self.node_feature_selector[ntype]].values
                        ).float()
                    ...
                    try:
                        ...
                        for etype, (src, dst) in edge_dict.items():
                            ...
                            if self.edge_reverse:
                                ...
                                edge_feat_df = edge_feat_df.rename(
                            ...
                            edge_feat_df = edge_feat_df.reindex(
                                pd.MultiIndex.from_arrays(
                                    [src, dst], names=["src", "dst"]
                                )
                            )
                            edge_feats[etype] = (
                                th.tensor(edge_feat_df["count"].values)
                                .unsqueeze(dim=1)
                                .float()
                            )
                    except Exception as e:
                ...
                if file_missing:
                    logger.warning(
                        f"Missing feature files for timestamp {timestamp_at_minute} ({time_str}), using zero values."
                    )
                if self.is_mask:
                    mask_prepare(edge_dict, num_nodes_dict, node_feats, edge_feats)
                graph = dgl.heterograph(
                    data_dict=edge_dict, num_nodes_dict=num_nodes_dict
                )
        ...
        new_labels = []
        for i, label in tqdm(
            enumerate(self.labels), desc="Aggregating fault data for each case..."
        ):
        ...
        if len(self.graphs) == 0:
            logger.error(
                f"No graphs loaded for dates {self.dates} with failure_types {self.failure_types}. "
                f"Please check if the test dates have matching failure data or if failure_types are correct."
            )
            raise ValueError(
                f"No graphs loaded. Check test dates and failure_types configuration."
            )
        self.get_nan_nodes()
        self.get_nan_edges()
    ...
    def scale(
        ...
        self.node_scale(
            node_scalers=node_scalers, attr="feat", node_scale_type=node_scale_type
        )
        self.edge_scale(
            edge_scalers=edge_scalers, attr="feat", edge_scale_type=edge_scale_type
        )
    ...
    def inverse_scale(self, feats, type="nodewise", log=True):
        """Reverse scales the features back to their original scale.
        Args:
            feats (dict): Features of a single sample.
        Returns:
            feats_inv (dict): Features after reverse scaling.
        """
        feats_inv = {}
        if type == "nodewise":
            for ntype, feat in feats.items():
    ...
    def get_nan_nodes(self):
        ...
        for ntype in self.graphs[0].ntypes:
            ...
            for g in self.graphs:
                ...
                nan = th.where(
                    th.isnan(data).all(dim=1), th.tensor(True), th.tensor(False)
                )
    ...
    def process_missing_values(self):
        ...
        def interp_numpy(arr):
            ...
            arr[isnan] = np.interp(
                np.flatnonzero(isnan), np.flatnonzero(~isnan), arr[~isnan]
            )
            return arr
    ...
    def generate_hash_id(self):
        ...
        failure_types_str = (
            "".join(self.failure_types) if self.failure_types is not None else ""
        )
        data_str = "".join(
            [
                str(self.data_dir),
