...
import json
try:
    from tqdm import tqdm  # pyright: ignore[reportMissingModuleSource]  # pyright: ignore[reportMissingModuleSource]
except ImportError:
...
from dgl import DGLGraph  # pyright: ignore[reportMissingImports]
from dgl.nn.pytorch import GATConv  # pyright: ignore[reportMissingImports]
...
# 添加路径以便导入causelens模块
try:
    from mask import load_mask_feat, replace_node_feats  # type: ignore[import-untyped]
    from utils import euclidean, mahalanobis, load_stats  # type: ignore[import-untyped]
    from log import Logger  # type: ignore[import-untyped]
    from pcmci_plus_runner import run_pcmci_plus_from_stacked  # type: ignore[import-untyped]
    # 导入成功，创建 logger 实例
...
# 延迟导入tigramite
try:
    from tigramite.data_processing import DataFrame  # type: ignore[import-untyped]
    from tigramite.independence_tests.parcorr import ParCorr  # type: ignore[import-untyped]
    from tigramite.pcmci import PCMCI  # type: ignore[import-untyped]
except ImportError:
    DataFrame = None
    ParCorr = None
    PCMCI = None
...
def constrained_pcmci_plus(
    ...
    """
    约束式PCMCIplus：在trace拓扑约束下运行，获取时滞分布先验。
    
    Args:
        g: DGL图（trace拓扑约束）
        stacked_nfeat: 堆叠特征 {(T, N, F)}
        labels: 标签列表
        ntype: 节点类型
        feat_idx: 特征索引
        tau_max: 最大时滞
    ...
    Returns:
        时滞显著性分布 P_prior: {(u, v): {lag: significance_score}}
    """
    logger.info(f'Stage 1.1: 运行约束式PCMCIplus (tau_max={tau_max})...')
    
    # 获取图的边作为约束（仅在存在调用关系的边上运行PCMCIplus）
    constrained_edges = set()
    for etype in g.canonical_etypes:
        if etype[2] == ntype:  # 目标节点类型匹配
            src, dst = g.edges(etype=etype)
            for u, v in zip(src.tolist(), dst.tolist()):
                constrained_edges.add((u, v))
    
    logger.info(f'图约束边数量: {len(constrained_edges)}')
    
    # 运行PCMCIplus获取所有时滞的因果强度
    try:
        ts, keep_mask = _select_nodes_for_pcmci(stacked_nfeat, nan_nodes, ntype, feat_idx, None)
        ...
        for to_idx in range(n_vars):
            for frm_idx in range(n_vars):
                ...
                for lag in range(0, tau_max + 1):
                    ...
                    sig = q_matrix[to_idx, frm_idx, lag] if q_matrix is not None else p_matrix[to_idx, frm_idx, lag]
                    
                    if sig is not None and not np.isnan(sig) and sig <= alpha:
                        # 使用绝对值的偏相关系数作为显著性分数
                        significance_score = abs(float(val)) * (1.0 - sig / alpha)  # sig越小，分数越高
...
def _select_nodes_for_pcmci(stacked_nfeat, nan_nodes, ntype: str, feat_idx: int, max_nodes: Optional[int]):
    """从stacked_nfeat中提取时间序列矩阵（用于PCMCIplus）。"""
    try:
        from pcmci_plus_runner import _select_nodes  # type: ignore[import-untyped]
...
def constrained_pcmci_plus_with_timeseries(
    ...
    """
    约束式PCMCIplus（使用服务级时间序列）：在trace拓扑约束下运行，获取时滞分布先验。
    
    Args:
        service_timeseries: 服务级时间序列 (T, N, F)
        service_list: 服务列表，与 service_timeseries 的第1维对应
        G_trace_edges: Trace拓扑边列表 [(service_u, service_v), ...]
        tau_max: 最大时滞
        alpha: 显著性水平
        feat_idx: 特征索引（使用第几维特征）
    
    Returns:
        时滞显著性分布 P_prior: {(u_idx, v_idx): {lag: significance_score}}
        其中 u_idx, v_idx 是服务在 service_list 中的索引
    """
    logger.info(f'Stage 1.1: 运行约束式PCMCIplus（服务级时间序列）(tau_max={tau_max})...')
    ...
    logger.info(f'Trace拓扑约束边数量: {len(constrained_edges)}')
    
    # 从 service_timeseries 中提取时间序列矩阵
    # service_timeseries: (T, N, F)
    ts = service_timeseries[:, :, feat_idx].detach().cpu().numpy()  # (T, N)
...
def historical_window_encoding(
    ...
    对每个服务s，当前窗口t，取历史窗口序列[t-tau_max, t]。
    差分编码: H_s^k = Encoder(X_s^{t-k}) - Encoder(X_s^{norm})
    ...
    for ntype in stacked_nfeat.keys():
        H[ntype] = {}
        feat_seq = stacked_nfeat[ntype]  # (T, N, F)
        norm_feat = data_stats.get('mean', {}).get(ntype, None)
        
        # 获取历史窗口 [max(0, i-tau_max), i+1]
        ...
        for lag in range(tau_max + 1):
            ...
            if hist_idx < 0 or hist_idx >= feat_seq.shape[0]:
                ...
                # 当前时刻特征
                X_t_k = feat_seq[hist_idx].to(device)  # (N, F)
                
                # 归一化特征（如果可用）
                if norm_feat is not None:
                    X_norm = norm_feat.to(device)  # (N, F) 或标量
                    if X_norm.dim() == 1 and X_norm.shape[0] == X_t_k.shape[0]:
                        # 每个节点的归一化特征
                        H_diff = X_t_k - X_norm.unsqueeze(1) if X_norm.dim() == 1 else X_t_k - X_norm
                    else:
                        # 全局归一化特征
                        H_diff = X_t_k - X_norm
                else:
        ...
        # 当前时刻（lag=0）
        #h0 = feat_seq[i].to(device) if i < feat_seq.shape[0] else th.zeros_like(feat_seq[0]).to(device)
        #if th.isnan(h0).any():
            #h0 = th.where(th.isnan(h0), th.zeros_like(h0), h0)
        #H[ntype][0] = h0
...
def _safe_minmax_norm(scores: Optional[th.Tensor]) -> Optional[th.Tensor]:
    """Min-max normalize a 1D tensor while staying stable on constant or empty inputs."""
    if scores is None:
        return None
    if scores.numel() == 0:
        return scores
...
def _stable_rms_norm(values: th.Tensor) -> float:
    """Return a scale-stable RMS norm for feature deltas."""
    if values.numel() == 0:
        return 0.0
    values = values.float()
...
def _combine_score_tensors(
    ...
    """
    Normalize and combine multiple 1D score tensors.
    Each item is (name, scores, weight).
    """
    ...
    fused = th.zeros_like(valid_items[0][1], dtype=th.float32, device=valid_items[0][1].device)
...
def _transform_stage2_score_values(
    ...
    """Apply a robust monotonic transform to Stage 2 scores before fusion."""
    ...
    if normalized_transform in ('clip', 'log1p_clip') and len(clean_values) >= max(int(clip_min_candidates), 1):
...
def _transform_stage2_score_pairs(
    ...
    """Transform a ranked Stage 2 score list while keeping it sorted descending."""
    if not score_pairs:
        return [], {
            'transform': str(transform),
            'clip_percentile': float(clip_percentile),
...
def _aggregate_candidate_records(
    ...
    """Aggregate stage-1 candidates across samples using stable global signals.

    A light auxiliary boost is added for weak fault families only when a
    candidate repeatedly appears in channel-B across those samples. This keeps
    the shared pool mostly driven by global agreement while still giving
    stress/network/pod-failure candidates a small lift.
    """
    ...
    for cid in candidate_ids:
        ...
        focus_vote_scores.append(float(fcnt) / max(focus_samples, 1) if focus_samples > 0 else 0.0)
        focus_mean_scores.append(float(focus_sum.get(cid, 0.0)) / max(fcnt, 1) if fcnt > 0 else 0.0)
    ...
    if fused is None:
        return sorted(candidate_ids, key=lambda cid: cand_cnt[cid], reverse=True)[:top_k]
...
class LagAwareAttentionLayer(nn.Module):
    ...
    在聚合邻居u的信息传给v时，计算时间维度的注意力权重α_{u,v}^k
    """
    ...
    def forward(
        self,
        H_v_0: th.Tensor,  # 当前节点v的当前特征 (N_v, F)
        H_u_lags: Dict[int, th.Tensor],  # 邻居u的历史特征 {lag: (N_u, F)}
        P_prior: Optional[Dict[int, float]] = None,  # PCMCI先验分布 {lag: prob}
    ) -> Tuple[th.Tensor, Dict[int, float]]:
        """
        计算软时滞注意力并聚合。
        
        Returns:
            h_v_agg: 聚合后的特征 (N_v, out_feats)
            alpha_dist: 注意力分布 {lag: weight}
        """
        if not H_u_lags:
        ...
        for lag in lags:
            H_u_k = H_u_lags[lag]  # (N_u, F)
            
            # 简化的注意力计算（实际应该考虑所有(u,v)对）
            # 这里假设H_u_k和H_v_0已经对齐或可以广播
            if H_u_k.shape[0] == H_v_0.shape[0]:
                concat_feat = th.cat([H_v_0, H_u_k], dim=-1)  # (N, 2*F)
            else:
                # 广播或平均池化
                if H_u_k.shape[0] > H_v_0.shape[0]:
            ...
            # 注意力分数: Attn(H_v^0, H_u^k)
            score = self.attn_linear(concat_feat).mean(dim=0)  # (num_heads,)
        ...
        # Softmax归一化
        scores = th.stack([score for _, score in attn_scores], dim=0)  # (num_lags, num_heads)
        alpha = F.softmax(scores, dim=0)  # (num_lags, num_heads)
        ...
        for idx, (lag, _) in enumerate(attn_scores):
            weight = alpha[idx].mean().item()  # 平均多头注意力
...
def _aggregate_prior_by_lag(
    ...
    """
    将 constrained PCMCIplus 返回的 P_prior[(u,v)]{lag:prob} 聚合成 {lag:prob}。

    这样 LagAwareAttentionLayer 才能基于先验分布对不同 lag 做 soft 引导。
    """
    if not P_prior:
        return None

    # 跨所有约束边把 {lag: prob} 聚合成一个全局分布 {lag: prob}。
...
class LagAwareGNN(nn.Module):
    ...
    def __init__(
        ...
        # 第一层：时滞注意力层
        self.layers.append(LagAwareAttentionLayer(in_feats, hidden_feats, tau_max, num_heads))
        
        # 后续层：标准GAT层
        for _ in range(num_layers - 1):
            self.layers.append(GATConv(hidden_feats, hidden_feats, num_heads))
        
        # 输出层：GAT层输出是 (N, num_heads, hidden_feats)，展平后是 (N, num_heads * hidden_feats)
    ...
    def forward(
        ...
        """
        前向传播，返回节点表示和修正后的主导时滞。
        
        Returns:
            node_repr: {ntype: (N, out_feats)}
            tau_star: {(u, v): dominant_lag}
        """
        # 简化的实现（实际需要完整的图遍历）
        # 自动检测节点类型（支持'api'和'pod'）
        ntype = None
        ...
        for i in range(1, len(self.layers)):
            ...
            if isinstance(x, tuple):
                x = x[0]
            # GAT层输出是 (N, num_heads, hidden_feats)，需要展平
            if x.dim() == 3:
                x = x.view(x.shape[0], -1)  # (N, num_heads * hidden_feats)
            else:
                x = x.view(x.shape[0], -1)  # 已经是展平的
        
        # 输出投影
        # 如果经过了GAT层，x的维度是 num_heads * hidden_feats
        # 如果只有LagAwareAttentionLayer，x的维度是 hidden_feats
        node_repr = {ntype: self.output_proj(x)}
        
        # 生成 tau_star：Stage2 会用 (u, v) 中的 v 对齐 service_id
        # 这里使用同一个 dominant_lag 给所有 service（仍比默认固定 1 更“有依据”）。
...
def _rerank_shared_with_stage1(
    ...
    """Fuse shared stage-2 ranking with stage-1 evidence."""
    ...
    sorted_idx = sorted(range(len(combined_list)), key=lambda i: combined_list[i], reverse=True)
...
def _build_per_sample_final_rankings(
    ...
    """Build per-sample rankings from sample-specific Stage-1 evidence plus shared Stage-2 scores."""
    ...
    for rec in stage1_per_sample:
        ...
        for pid in sample_pool:
            sample_stage1_vals.append(float(stage1_scores[pid]) if 0 <= pid < len(stage1_scores) else 0.0)
            sample_stage2_vals.append(float(stage2_score_map.get(pid, 0.0)))

        stage1_norm_tensor = _safe_minmax_norm(th.tensor(sample_stage1_vals, dtype=th.float32))
        stage2_norm_tensor = _safe_minmax_norm(th.tensor(sample_stage2_vals, dtype=th.float32))
        ...
        if combined_tensor is None:
            final_ranking = [(pid, stage2_score_map.get(pid, 0.0)) for pid in sample_pool[:top_k_pods]]
...
def _score_stage2_for_candidate_pool(
...
) -> Union[List[Tuple[int, float]], Tuple[List[Tuple[int, float]], Dict[int, Dict[str, float]]]]:
    """Compute Stage-2 scores while shrinking duplicated bonus terms conservatively."""
    ...
    for service_id in candidate_ids:
        ...
        if th.isnan(service_norm).any():
            service_norm = th.where(th.isnan(service_norm), th.zeros_like(service_norm), service_norm)
        ...
        for pod_id, pod_hist_feats in service_pods.items():
            pod_feat_tau = pod_hist_feats.get(service_tau_star, pod_hist_feats.get(0, service_norm))
            if th.isnan(pod_feat_tau).any():
                pod_feat_tau = th.where(th.isnan(pod_feat_tau), th.zeros_like(pod_feat_tau), pod_feat_tau)

            delta_Z_p = pod_multidimensional_anomaly_modeling(pod_feat_tau, service_tau_star, service_norm)
            if th.isnan(delta_Z_p).any():
                delta_Z_p = th.where(th.isnan(delta_Z_p), th.zeros_like(delta_Z_p), delta_Z_p)

            delta_norm = float(delta_Z_p.norm().item())
            if np.isnan(delta_norm) or np.isinf(delta_norm):
                model_score = 0.0
            else:
                model_score = float(th.sigmoid(th.tensor(delta_norm)).item() * delta_norm)
            if not local_anomaly_score_enabled:
                model_score = 0.0

            downstream_feat = downstream_feats.get(service_id, th.zeros_like(pod_feat_tau))
            if th.isnan(downstream_feat).any():
                downstream_feat = th.where(th.isnan(downstream_feat), th.zeros_like(downstream_feat), downstream_feat)

            counterfactual_effect = counterfactual.counterfactual_effect(
                pod_id, service_id, pod_hist_feats, service_norm, downstream_feat, service_tau_star
            )
...
def _compute_top1_challenger_features(
    ...
    """Build a small set of source/local/lag diagnostics for top-1 pairwise comparison."""
    ...
    if owner_service_id is None:
        return {
            ...
            'local_anomaly': 0.0,
            'lag_consistency': 0.0,
            'stage1_score': float(sample_scores[pod_id]) if 0 <= pod_id < len(sample_scores) else 0.0,
        }
    ...
    downstream_feat = downstream_feats.get(owner_service_id, th.zeros_like(pod_feat_tau))
    ...
    if th.isnan(pod_feat_t0).any():
        pod_feat_t0 = th.where(th.isnan(pod_feat_t0), th.zeros_like(pod_feat_t0), pod_feat_t0)
    if th.isnan(downstream_feat).any():
        downstream_feat = th.where(th.isnan(downstream_feat), th.zeros_like(downstream_feat), downstream_feat)
...
def _compute_source_first_signal(
    ...
    """Reward candidates whose anomaly rises earlier in the history window than sink-like responders."""
    ...
    earliest_sig_lag = max(significant_lags) if significant_lags else max(lag for lag, _ in lag_scores)
    ...
    return max(0.55 * earlier_than_current + 0.45 * peak_alignment - 0.75 * current_sink_bias, 0.0)
...
def _build_per_sample_final_rankings_true_stage2(
    ...
    """Build final rankings by recomputing Stage-2 for each sample's own candidate pool."""
    results: List[Dict] = []
    ...
    for rec in stage1_per_sample:
        ...
        if primary_pool:
            s1_vals = [float(sample_scores[pid]) if 0 <= pid < len(sample_scores) else 0.0 for pid in primary_pool]
            ...
            if combined_tensor is None:
                primary_ranking = [(pid, stage2_map.get(pid, 0.0)) for pid in primary_pool]
            else:
                combined = [float(x) for x in combined_tensor.tolist()]
                ...
                # Control variant:
                # leave the primary ranking as the direct stage1+stage2 fusion result,
                # without any extra head bias or challenger-based top1 flipping.

        fallback_ranking: List[Tuple[int, float]] = []
        if fallback_pool:
            fallback_scores = [float(score) * 0.85 for _, score in fallback[:top_k_pods] if int(_) in fallback_pool]
            fallback_map = {}
...
def pod_multidimensional_anomaly_modeling(
    ...
    if th.isnan(norm_feats).any():
        norm_feats = th.where(th.isnan(norm_feats), th.zeros_like(norm_feats), norm_feats)
...
class LagCalibratedCounterfactual:
    ...
    def counterfactual_effect(
        ...
        pod_id: int,
        service_id: int,
        Z_p_hist: Dict[int, th.Tensor],  # {lag: features}
        Z_p_norm: th.Tensor,
        ...
        Args:
            pod_id: Pod ID
            service_id: 服务ID
            Z_p_hist: Pod历史特征 {lag: (F,)}
            Z_p_norm: Pod归一化特征
            downstream_feats: 下游特征
            tau_star_lag: 主导时滞
        
        Returns:
            CounterfactualEffect: 反事实效应分数
        """
        # Factual Prediction: 使用真实Pod状态预测下游
        ...
        if th.isnan(Z_p_factual).any():
            Z_p_factual = th.where(th.isnan(Z_p_factual), th.zeros_like(Z_p_factual), Z_p_factual)
        if th.isnan(Z_p_norm).any():
            Z_p_norm = th.where(th.isnan(Z_p_norm), th.zeros_like(Z_p_norm), Z_p_norm)
        
        # 简化的预测器调用（实际需要完整的模型forward）
        # Y_fact = self.predictor(Z_p_factual, downstream_feats)
        
        # Lag-Calibrated Intervention: 在t-τ*时刻恢复正常
        Z_p_intervened = Z_p_norm
        
        # Counterfactual Prediction
        # Y_cf = self.predictor(Z_p_intervened, downstream_feats)
        
        # 效应计算（简化）
        # effect = (Y_fact - Y_cf).norm().item()
...
def stage2_fine_grained_localization(
    S_cand: List[int],
    tau_star: Dict[Tuple[int, int], int],
    pod_feats: Dict[int, Dict[int, th.Tensor]],  # {service_id: {pod_id: {lag: features}}}
    norm_pod_feats: Dict[int, th.Tensor],
    downstream_feats: Dict[int, th.Tensor],
    ...
    Args:
        S_cand: Stage 1的候选服务列表
        tau_star: 主导时滞 {(u, v): lag}
        pod_feats: Pod特征 {service_id: {pod_id: {lag: features}}}
        norm_pod_feats: 归一化Pod特征
        downstream_feats: 下游特征
        predictor_model: 预测器模型（可选）
    
    Returns:
        候选Pod列表 [(pod_id, score), ...]
    """
    ...
    for service_id in S_cand:
        ...
        for pod_id, pod_hist_feats in service_pods.items():
            ...
            if np.isnan(delta_norm) or np.isinf(delta_norm):
                ...
                model_score = float(np.log1p(th.sigmoid(th.tensor(delta_norm)).item() * delta_norm))
            if not local_anomaly_score_enabled:
                model_score = 0.0
            ...
            counterfactual_effect = counterfactual.counterfactual_effect(
                pod_id, service_id, pod_hist_feats, service_norm,
                downstream_feat, service_tau_star
            )
            
            # 检查counterfactual_effect中的NaN
            if np.isnan(counterfactual_effect) or np.isinf(counterfactual_effect):
                counterfactual_effect = 0.0
...
def node_confounder_pruning(
    ...
    """
    节点共因剔除：若PCMCIplus检测到Pod内部N→Z_p相关性且无显著时滞，则下调根因概率。
    
    Args:
        pod_scores: Pod得分列表
        pcmci_results: PCMCIplus结果
        threshold: 时滞阈值（小于此值视为无显著时滞）
    ...
    for pod_id, score in pod_scores:
        # 检查该Pod是否有节点内部的相关性且时滞很小
        # 简化实现：如果PCMCI显示lag=0或lag=1的相关性，下调分数
        should_prune = False
        
        # 实际应该检查pcmci_results中该Pod的相关性
        # 这里简化处理
...
# ============================================================================
# Active RCA entrypoint
# NOTE:
# This is the only implementation that should be edited for current experiments.
# Earlier lag_aware_dual_stage_rca definitions are legacy snapshots kept only to
# avoid a large cleanup during metric iteration.
# ============================================================================
def _normalize_failure_type(label: Optional[Dict]) -> str:
    """Normalize failure type into a compact routing key."""
...
def _channel_profile_from_failure_type(failure_type: str) -> Dict[str, float]:
    """Choose stage-1 recall emphasis by failure type."""
...
def _extract_temporal_response_scores_v2(
    ...
    """Fault-type-aware temporal response score."""
    ...
    delta_norm = (focus_current - focus_prev).abs().norm(dim=1) * float(profile.get('delta_scale', 1.0))
    ...
    peak_norm = None
    if lag_energies:
        peak_norm = th.stack(lag_energies, dim=0).max(dim=0).values * float(profile.get('peak_scale', 1.0))
...
def _stage1_source_aware_profile(failure_type: str) -> Dict[str, float]:
    """Mechanism-level weights for Stage1 source-aware score shaping."""
    ...
    elif any(key in ft for key in ("container-kill", "pod-failure", "pod-kill", "kill", "oom")):
        profile.update({
...
def _compute_stage1_source_aware_terms(
    ...
    """Compute query-local source-like evidence from lagged node features.

    The terms are deliberately mechanism-generic: early anomaly onset and local
    dominance are rewarded, while current-only sink-like spikes are penalized.
    """
    if ntype not in H or 0 not in H[ntype]:
        return {"source_first": None, "local_margin": None, "current_change": None, "current_drop": None, "sink_penalty": None}

    current_feat = H[ntype][0]
    if th.isnan(current_feat).any():
        current_feat = th.where(th.isnan(current_feat), th.zeros_like(current_feat), current_feat)
    ...
    if th.isnan(prev_energy).any():
        prev_energy = th.where(th.isnan(prev_energy), th.zeros_like(prev_energy), prev_energy)
    ...
    source_first = _safe_minmax_norm(0.75 * earlier_than_current + 0.25 * sustained_history)

    local_center = current_energy.mean() if current_energy.numel() > 0 else th.tensor(0.0, device=current_energy.device)
    ...
    current_drop = _safe_minmax_norm(th.clamp(prev_abs_mean - current_abs_mean, min=0.0))
    ...
    return {
        ...
        "current_change": current_change.to(device) if current_change is not None else None,
...
def stage1_fault_type_dual_channel_localization(
    ...
    """Stage 1 with fault-type-aware dual-channel union recall."""
    ...
    weak_expanded_top_k = max(top_k, int(round(top_k * max(weak_type_expand_factor, 1.0))))
    weak_type_set = {
        str(ft).strip().lower()
        for ft in (weak_failure_types or [])
        if str(ft).strip()
    }
    ...
    for i, g in enumerate(tqdm(graphs, desc='Stage 1 typed dual', total=len(labels))):
        ...
        aux_sample = None
        if stage1_channel_aux is not None and stage1_channel_aux.ndim == 3 and i < int(stage1_channel_aux.shape[0]):
        ...
        if ntype in H and 0 in H[ntype]:
            ...
            if temporal_response_channel_enabled and channel_b_scores is not None and anomaly_scores is not None:
                typed_response_weight = float(profile.get('response_weight', response_weight))
                channel_b_scores = _combine_score_tensors(
                    [
                        ('response', channel_b_scores, typed_response_weight),
                        ('anomaly', anomaly_scores, max(1.0 - typed_response_weight, 0.0)),
                    ],
                    device=device,
                )
            ...
            k_a = min(expanded_top_k, int(channel_a_scores.shape[0])) if channel_a_scores is not None else 0
            k_b = min(expanded_top_k, int(channel_b_scores.shape[0])) if temporal_response_channel_enabled and channel_b_scores is not None else 0
            if k_a > 0:
                top_a = channel_a_scores.topk(k_a).indices.tolist()
            if k_b > 0:
                top_b = channel_b_scores.topk(k_b).indices.tolist()

            typed_bonus = float(profile.get('dual_hit_bonus', dual_hit_bonus)) if dual_channel_consistency_enabled else 0.0
            ...
            for pid in union_candidate_ids:
                a_val = float(channel_a_scores[pid]) if channel_a_scores is not None and 0 <= pid < len(channel_a_scores) else 0.0
                b_val = float(channel_b_scores[pid]) if channel_b_scores is not None and 0 <= pid < len(channel_b_scores) else 0.0
                bonus = typed_bonus if hit_counts.get(int(pid), 0) >= 2 else 0.0
                union_values.append(max(a_val, b_val) + bonus)
            union_norm = _safe_minmax_norm(th.tensor(union_values, dtype=th.float32)) if union_values else None
            if source_aware_enabled and union_values and union_norm is not None:
                ...
                source_first_w = float(source_profile.get('source_first', 0.10)) * strength
                local_margin_w = float(source_profile.get('local_margin', 0.10)) * strength
                current_change_w = float(source_profile.get('current_change', 0.06)) * strength
                current_drop_w = float(source_profile.get('current_drop', 0.0)) * strength
                sink_penalty_w = float(source_profile.get('sink_penalty', 0.06)) * strength
                source_terms = _compute_stage1_source_aware_terms(H, ntype, device=device)
            ...
            union_scores = th.zeros(total_nodes if total_nodes > 0 else len(union_candidate_ids), dtype=th.float32)
            if union_norm is not None:
                for local_idx, pid in enumerate(union_candidate_ids):
                    if pid >= union_scores.shape[0]:
                        pad = th.zeros(pid - union_scores.shape[0] + 1, dtype=th.float32)
                        union_scores = th.cat([union_scores, pad], dim=0)
                    union_scores[pid] = union_norm[local_idx]
            union_candidates = [pid for _, pid in sorted(zip(union_values, union_candidate_ids), reverse=True)][:expanded_top_k]
        ...
        per_sample_results.append(
            {
                ...
                'conditional_expansion': {
                    ...
                    'applied': bool(weak_type_conditional_expansion_enabled and failure_type in weak_type_set),
    ...
    if not S_cand:
        candidate_counts = {c: global_candidate_votes.count(c) for c in set(global_candidate_votes)}
        S_cand = sorted(candidate_counts.keys(), key=lambda x: candidate_counts[x], reverse=True)[:top_k]

    logger.info(f'Stage 1 fault-type-aware recall completed, retained {len(S_cand)} candidates')
...
def _compute_relative_source_signal(
    ...
    """Compute relative source-ness between candidates instead of isolated local strength."""
        ...
        if lag_alignment_enabled:
            for (u, v), lag in tau_star.items():
                if v == service_id:
                    service_tau = lag
                    break
        current_feat = pod_hist_feats.get(service_tau, pod_hist_feats.get(0, service_norm))
        prev_feat = pod_hist_feats.get(min(service_tau + 1, max(pod_hist_feats.keys())), service_norm)
        base_self[pid] = float((current_feat - service_norm).abs().norm().item())
        base_delta[pid] = float((current_feat - prev_feat).abs().norm().item())
        base_downstream[pid] = float((downstream_feat - service_norm).abs().norm().item())
    ...
    for pid in candidate_ids:
        ...
        if others:
            other_self_mean = float(np.mean([base_self.get(oid, 0.0) for oid in others]))
            other_delta_mean = float(np.mean([base_delta.get(oid, 0.0) for oid in others]))
        else:
            other_self_mean = 0.0
        ...
        downstream_margin = max(base_self.get(pid, 0.0) - base_downstream.get(pid, 0.0), 0.0)
    ...
    norm_tensor = _safe_minmax_norm(th.tensor([raw_scores[pid] for pid in candidate_ids], dtype=th.float32))
    if norm_tensor is None:
        return {pid: 0.0 for pid in candidate_ids}, diagnostics
    return {pid: float(norm_tensor[idx]) for idx, pid in enumerate(candidate_ids)}, diagnostics
...
def _relative_source_rerank(
    ...
    keep_top1_anchor: bool,
    anchor_margin: float,
) -> Tuple[List[Tuple[int, float]], List[Dict], List[float], List[float], List[float], List[int]]:
    """Rerank candidates using relative source advantage among the candidate set."""
    if not candidate_pool:
        return [], [], [], [], [], []

    shared_candidate_pool, shared_stage1_vals, shared_stage2_vals, vote_counts = _rerank_shared_with_stage1(
    ...
    shared_scores = [float(x) for x in combined.tolist()] if combined is not None else [0.0 for _ in pid_list]
    if keep_top1_anchor and shared_scores:
        shared_scores[0] = max(shared_scores) + anchor_margin
    final_shared = sorted(zip(pid_list, shared_scores), key=lambda item: item[1], reverse=True)
    ...
    for rec in stage1_per_sample or []:
        ...
        for pid in pid_list:
            if isinstance(rec_scores, list) and 0 <= pid < len(rec_scores):
                ...
                sample_stage1_vals.append(0.0)
        sample_stage1_norm = _safe_minmax_norm(th.tensor(sample_stage1_vals, dtype=th.float32))
        ...
        sample_scores = [float(x) for x in sample_combined.tolist()] if sample_combined is not None else [0.0 for _ in pid_list]
        ranked = sorted(zip(pid_list, sample_scores), key=lambda item: item[1], reverse=True)
        per_sample_results.append({'final_ranking': ranked[:top_k_pods]})

    return final_shared, per_sample_results, shared_stage1_vals, shared_stage2_vals, shared_source_vals, vote_counts


def _failure_type_rerank_profile(failure_type: str) -> Dict[str, float]:
    """按故障类型返回 type-aware 融合系数（相对 CLI 默认权重的乘性偏置思路）。

    返回值键含义（与 ``_failure_type_aware_rerank`` 内一致）：
    - ``stage1`` / ``stage2`` / ``source``：各通道在融合里的权重（会覆盖该样本的默认 stage1/stage2/source）。
    - ``shared_penalty``：与 ``default_shared_penalty`` 相乘后，惩罚「多样本共享」候选；**越大**越压低热门 pod。
    - ``keep_anchor``：>0 时在融合后强行抬高当前 pool 第一名（与 ``keep_top1_anchor`` 逻辑与）。

    对照表（分支名 → 子串匹配 → 建议调参优先级）::

        分支 A「代码/异常形态」 replace-code, replace-method,
            replace-path, exception, return（replace-body 已单独分支）
            当前: stage1↓ stage2↑ source 适中, shared_penalty 中高
            若 top-3/5 尚可但 top-1 差: 先微调 shared_penalty↓ 或略升 stage1（易与 code challenger 联动）
            评估偏弱示例: exception, replace-path, return（样本少，需结合 rerank_debug 再改）

        分支 B「资源/杀容器」 stress, request-abort, pod-failure, container-kill
            当前: stage1↑ stage2↓, shared_penalty 很高, keep_anchor 关
            若漏召: 检查 Stage1 召回；若排序偏后: 可略升 stage2 或略降 shared_penalty

        分支 C「response-abort」
            当前: 介于延迟与代码之间, shared_penalty 中高

        分支 D「延迟类」 request-delay, response-delay
            当前: shared_penalty 低（少压共享候选）— 与强基线类型一致
            若过拟合全局热点: 可略升 shared_penalty

        分支 E「网络/存储/杂项」 partition, bandwidth, loss, mysql, corrupt, time, unknown
            当前: stage1↑ stage2↓, keep_anchor 关
            评估偏弱示例: corrupt, mysql, partition（优先确认真根因是否在候选池内）

        默认: 其余 failure_type
            当前: 均衡 profile

    弱类型调权优先级（在对比实验锁定瓶颈后）：先调 ``shared_penalty`` 与 stage1/stage2 比例，再动 ``source``；
    候选常不在 top-K 时优先 **主流程召回**（top_k_services / expand_factor / reranker_candidate_pool），而非仅改本表。
    """
    ft = str(failure_type or '').strip().lower()

    # replace-body: reranker 易压扁真值 pod 的 stage2；提高 stage1、略降 stage2，减轻对融合后 stage2 的依赖。
    if 'replace-body' in ft:
        return {'stage1': 0.40, 'stage2': 0.38, 'source': 0.22, 'keep_anchor': 1.0, 'shared_penalty': 0.48}

    # Shared-candidate suppression should focus on failure types that are most
    # vulnerable to a globally strong pod monopolizing top-1.
    if any(key in ft for key in ('replace-code', 'replace-method', 'replace-path', 'exception', 'return')):
        return {'stage1': 0.20, 'stage2': 0.55, 'source': 0.25, 'keep_anchor': 1.0, 'shared_penalty': 0.65}

    if any(key in ft for key in ('stress', 'request-abort', 'pod-failure', 'container-kill')):
        return {'stage1': 0.55, 'stage2': 0.25, 'source': 0.20, 'keep_anchor': 0.0, 'shared_penalty': 1.65}

    if any(key in ft for key in ('response-abort',)):
        return {'stage1': 0.30, 'stage2': 0.45, 'source': 0.25, 'keep_anchor': 1.0, 'shared_penalty': 1.15}

    if any(key in ft for key in ('request-delay', 'response-delay')):
        return {'stage1': 0.30, 'stage2': 0.45, 'source': 0.25, 'keep_anchor': 1.0, 'shared_penalty': 0.20}

    if any(key in ft for key in ('partition', 'bandwidth', 'loss', 'mysql', 'corrupt', 'time', 'unknown')):
        return {'stage1': 0.60, 'stage2': 0.20, 'source': 0.20, 'keep_anchor': 0.0, 'shared_penalty': 0.75}

    return {'stage1': 0.40, 'stage2': 0.35, 'source': 0.25, 'keep_anchor': 1.0, 'shared_penalty': 0.85}
...
def _apply_code_family_top1_challenger(
    ...
    ranked_indices = sorted(range(len(pool)), key=lambda idx: combined_scores[idx], reverse=True)
    ...
    has_stronger_signal = (
        best_stage2 >= incumbent_stage2
        and best_source >= max(incumbent_source - 0.02, 0.0)
    )
...
def _is_top1_trap_failure(failure_type: str) -> bool:
    ft = str(failure_type or '').strip().lower()
    return any(key in ft for key in ('stress', 'request-abort', 'pod-failure', 'response-abort', 'container-kill'))
...
def _apply_global_hot_candidate_challenger(
    ...
    incumbent_hot = float(shared_vals[incumbent_idx]) if incumbent_idx < len(shared_vals) else 0.0
    incumbent_src = _norm_at(source_norm, incumbent_idx)
    ...
    return adjusted_scores, {
        ...
        'incumbent_hot_score': incumbent_hot,
        'challenger_hot_score': float(shared_vals[best_idx]) if best_idx < len(shared_vals) else 0.0,
        'incumbent_local_evidence': float(incumbent_local),
        'challenger_local_evidence': float(best_local),
        'incumbent_debiased_evidence': float(incumbent_debiased),
        'challenger_debiased_evidence': float(best_debiased),
        'final_gap_before': float(combined_scores[incumbent_idx]) - float(combined_scores[best_idx]),
    }
...
def _rerank_blend_for_failure_type(
    ...
    """LGBM stage2 混合系数：默认与全局 ``reranker_blend`` 一致；``replace-body`` 单独用更小 blend（更信主流程 stage2）。"""
    ft = str(failure_type or '').strip().lower()
    if 'replace-body' in ft:
...
def _apply_stage1_strong_evidence_guard(
    ...
    ranked_by_stage1 = sorted(range(n), key=lambda i: (-float(stage1_norm_vals[i]), int(pool[i])))
...
def _confidence_from_norm_values(values: Optional[th.Tensor], gap_topn: int) -> Dict[str, float]:
    if values is None:
...
def _apply_dynamic_confidence_weights(
    ...
    base_weights = np.asarray(
        [max(float(stage1_weight), 0.0), max(float(stage2_weight), 0.0), max(float(source_weight), 0.0)],
        dtype=np.float64,
    )
    total = float(base_weights.sum())
    if total <= 1e-12:
        return stage1_weight, stage2_weight, source_weight, {'enabled': True, 'applied': False, 'reason': 'zero_base_weight'}
    ...
    shift = np.clip(centered * float(max(strength, 0.0)), -float(max_shift), float(max_shift))
...
def _failure_type_aware_rerank(
    ...
    lgbm_rerank_pred_scaled: Optional[Union[Dict[int, float], List[Dict[int, float]]]] = None,
    ...
    max_shared_count = max(shared_candidate_counts.values()) if shared_candidate_counts else 0
    ...
    for idx, rec in enumerate(stage1_per_sample):
        ...
        shared_penalty_weight = float(profile.get('shared_penalty', 1.0)) * float(default_shared_penalty)
        local_keep_anchor = bool(profile.get('keep_anchor', 1.0)) and keep_top1_anchor

        base_ranking = per_sample_final[idx].get('final_ranking', []) if idx < len(per_sample_final) else []
        stage1_scores = rec.get('scores') or []
        sample_candidates = rec.get('candidate_services') or []
        ...
        if not pool:
            reranked_results.append({'final_ranking': [], 'debug_candidates': [], 'debug_weights': {}})
            continue
        ...
        if use_lgbm_per_type_blend and lgbm_rerank_pred_scaled is not None:
            if isinstance(lgbm_rerank_pred_scaled, list):
                if idx < len(lgbm_rerank_pred_scaled) and isinstance(lgbm_rerank_pred_scaled[idx], dict):
        ...
        lgbm_b = (
            _rerank_blend_for_failure_type(failure_type, lgbm_rerank_default_blend, lgbm_rerank_replace_body_blend)
        ...
        shared_vals = []
        for pid in pool:
            stage1_vals.append(float(stage1_scores[pid]) if isinstance(stage1_scores, list) and 0 <= pid < len(stage1_scores) else 0.0)
            pid_i = int(pid)
            if lgbm_pred_for_sample is not None and pid_i in lgbm_pred_for_sample:
                bb = float((lgbm_rerank_base_stage2 or {}).get(pid_i, stage2_score_map.get(pid_i, 0.0)))
            ...
            if max_shared_count > 0:
                shared_vals.append(float(shared_candidate_counts.get(pid, 0)) / float(max_shared_count))
            else:
        ...
        if dynamic_confidence_fusion_enabled:
            stage1_weight, stage2_weight, source_weight, dynamic_conf_debug = _apply_dynamic_confidence_weights(
                stage1_weight=stage1_weight,
                stage2_weight=stage2_weight,
        ...
        if combined is None:
            final_ranking = [(pid, stage2_score_map.get(pid, 0.0)) for pid in pool[:top_k_pods]]
            dw_fb = {
            ...
            reranked_results.append({
                ...
                'debug_candidates': [
                    {
                        'pod_id': int(pid),
                        'stage1_score': float(stage1_vals[pos]) if pos < len(stage1_vals) else 0.0,
                        'stage2_score': float(stage2_vals[pos]) if pos < len(stage2_vals) else 0.0,
                        'source_score': float(source_vals[pos]) if pos < len(source_vals) else 0.0,
        ...
        if shared_norm is not None and shared_penalty_weight > 0:
            shared_penalties = [float(x) * float(shared_penalty_weight) for x in shared_norm.tolist()]
            combined_scores = [score - penalty for score, penalty in zip(combined_scores, shared_penalties)]
        ...
        if stage1_guard_enabled:
            stage1_norm_vals = [float(x) for x in stage1_norm.tolist()] if stage1_norm is not None else []
        ...
        hot_challenger_debug: Dict[str, object] = {'promoted': False, 'reason': 'disabled'}
        if global_hot_challenger_enabled:
            combined_scores, hot_challenger_debug = _apply_global_hot_candidate_challenger(
                pool=pool,
                combined_scores=combined_scores,
                stage1_vals=stage1_vals,
                stage2_vals=stage2_vals,
            ...
            [
                {
                    ...
                    'stage2_score': float(stage2_vals[pos]) if pos < len(stage2_vals) else 0.0,
                    'source_score': float(source_vals[pos]) if pos < len(source_vals) else 0.0,
                    'shared_candidate_score': float(shared_vals[pos]) if pos < len(shared_vals) else 0.0,
                    'final_score': float(combined_scores[pos]) if pos < len(combined_scores) else 0.0,
                }
                for pos, pid in enumerate(pool)
            ],
        ...
        final_ranking = [(item['pod_id'], item['final_score']) for item in debug_candidates[:top_k_pods]]
        dw_ok = {
            'stage1': float(stage1_weight),
            'stage2': float(stage2_weight),
            'source': float(source_weight),
    ...
    shared_ranking = sorted(
        [(pid, float(np.mean(scores))) for pid, scores in shared_accumulator.items() if scores],
        key=lambda item: item[1],
        reverse=True,
    )[:top_k_pods]
...
def _leave_one_out_stage2_rerank(
    ...
    stage2_norm = [float(x) for x in stage2_norm_tensor.tolist()] if stage2_norm_tensor is not None else [0.0] * len(candidate_ids)
    ...
    for rec in stage1_per_sample:
        ...
        sample_candidate_ids = {
            int(pid) for pid in (rec.get('candidate_services') or [])
            if isinstance(pid, (int, np.integer)) or str(pid).isdigit()
        }
        s1_vals = [float(scores[pid]) if 0 <= pid < len(scores) else 0.0 for pid in candidate_ids]
        s1_norm_tensor = _safe_minmax_norm(th.tensor(s1_vals, dtype=th.float32))
        s1_norm = [float(x) for x in s1_norm_tensor.tolist()] if s1_norm_tensor is not None else [0.0] * len(candidate_ids)
        ...
        base_scores = [float(x) for x in base_tensor.tolist()] if base_tensor is not None else [0.0] * len(candidate_ids)
        base_norm_tensor = _safe_minmax_norm(th.tensor(base_scores, dtype=th.float32))
        base_norm = [float(x) for x in base_norm_tensor.tolist()] if base_norm_tensor is not None else [0.0] * len(candidate_ids)
    ...
    for holdout_idx, feats in enumerate(sample_features):
        ...
        for train_idx, train_feats in enumerate(sample_features):
            ...
            positive_shortlist = [idx for idx in shortlist if candidate_ids[idx] in targets and idx != incumbent_idx]
        ...
        if not train_x_list:
            ...
            ranked_idx = sorted(range(len(candidate_ids)), key=lambda idx: final_scores[idx], reverse=True)
            reranked_per_sample.append(
                {'final_ranking': [(candidate_ids[idx], final_scores[idx]) for idx in ranked_idx[:top_k_pods]]}
            )
        ...
        optimizer = th.optim.AdamW(model.parameters(), lr=float(lr), weight_decay=float(l2))
        ...
        if shortlist:
            ...
            for idx in shortlist[1:]:
                ...
                pair_feat = th.cat([cand_feat - incumbent_feat, extra], dim=0).unsqueeze(0)
                prob = float(th.sigmoid(model(pair_feat)).item())
                local_gain = (
                    0.55 * max(cand_s2 - incumbent_s2, 0.0)
                    + 0.35 * max(cand_s1 - incumbent_s1, 0.0)
                    + 0.10 * max(float(feats['base_norm'][idx]) - float(feats['base_norm'][incumbent_idx]), 0.0)
                )
                if prob > best_prob:
                    best_prob = prob
                    best_idx = idx
            ...
            if should_promote:
                ...
                final_scores[incumbent_idx] = min(incumbent_score, final_scores[best_idx] - 1e-4)
    ...
    shared_ranked_idx = sorted(range(len(candidate_ids)), key=lambda idx: shared_scores[idx], reverse=True)
    reranked_shared = [
...
def _lag_aware_dual_stage_rca_active(
    ...
    **kwargs,
    ...
    """Dual-stage RCA with fault-type-aware Stage 1 recall and type-aware Stage 2 reranking."""
    logger.info('=' * 60)
    ...
    if service_timeseries is not None and service_list is not None and G_trace_edges is not None:
    ...
    temporal_response_channel_enabled = bool(getattr(rca_config, 'temporal_response_channel_enabled', True))
    dual_channel_consistency_enabled = bool(getattr(rca_config, 'dual_channel_consistency_enabled', True))
    ...
    stage2_score_clip_percentile = getattr(rca_config, 'stage2_score_clip_percentile', 95.0)
    stage2_score_clip_min_candidates = getattr(rca_config, 'stage2_score_clip_min_candidates', 8)
    stage2_shared_candidate_penalty = float(getattr(rca_config, 'stage2_shared_candidate_penalty', 0.12))
    type_aware_rerank_enabled = bool(getattr(rca_config, 'type_aware_rerank_enabled', True))
    ...
    source_candidate_pool = max(int(getattr(rca_config, 'source_candidate_pool', max(top_k_pods, 15))), top_k_pods)
    ...
    reranker_candidate_pool = max(int(getattr(rca_config, 'reranker_candidate_pool', max(top_k_pods, 24))), top_k_pods)
    reranker_blend = getattr(rca_config, 'reranker_blend', 0.75)
    reranker_blend_replace_body = getattr(rca_config, 'reranker_blend_replace_body', None)
    ...
    code_top1_challenger_enabled = getattr(rca_config, 'code_top1_challenger_enabled', True)
    code_top1_challenger_topn = max(int(getattr(rca_config, 'code_top1_challenger_topn', 4)), 2)
    code_top1_challenger_margin = float(getattr(rca_config, 'code_top1_challenger_margin', 0.02))
    global_hot_challenger_enabled = getattr(rca_config, 'global_hot_challenger_enabled', False)
    global_hot_challenger_topn = max(int(getattr(rca_config, 'global_hot_challenger_topn', 5)), 2)
    global_hot_challenger_threshold = float(getattr(rca_config, 'global_hot_challenger_threshold', 0.95))
    global_hot_challenger_margin = float(getattr(rca_config, 'global_hot_challenger_margin', 0.30))
    global_hot_challenger_evidence_margin = float(getattr(rca_config, 'global_hot_challenger_evidence_margin', -0.05))
    ...
    stage1_guard_preserve_top1 = bool(getattr(rca_config, 'stage1_guard_preserve_top1', True))
    dynamic_confidence_fusion_enabled = bool(getattr(rca_config, 'dynamic_confidence_fusion_enabled', False))
    dynamic_confidence_strength = float(getattr(rca_config, 'dynamic_confidence_strength', 0.35))
    dynamic_confidence_gap_topn = max(int(getattr(rca_config, 'dynamic_confidence_gap_topn', 3)), 2)
    dynamic_confidence_max_shift = float(getattr(rca_config, 'dynamic_confidence_max_shift', 0.18))
    stage1_source_aware_score_enabled = bool(getattr(rca_config, 'stage1_source_aware_score_enabled', False))
    stage1_source_aware_strength = float(getattr(rca_config, 'stage1_source_aware_strength', 1.0))
    ...
    counterfactual_effect_enabled = bool(getattr(rca_config, 'counterfactual_effect_enabled', True))
    local_anomaly_score_enabled = bool(getattr(rca_config, 'local_anomaly_score_enabled', True))
    if not lag_prior_enabled:
        logger.info('Stage 1.1 prior ablation enabled: dropping lag-causal prior after skeleton discovery')
        P_prior = {}
    ...
    stage1_weak_type_expand_factor = float(getattr(rca_config, 'stage1_weak_type_expand_factor', 2.0))
    ...
    if isinstance(raw_weak_failure_types, str):
        stage1_weak_failure_types = [ft.strip() for ft in raw_weak_failure_types.split(',') if ft.strip()]
    else:
    ...
    relative_shared_ranking, per_sample_final, stage1_vals, stage2_vals, source_vals, vote_counts = _relative_source_rerank(
        candidate_pool=candidate_pool,
        stage1_per_sample=stage1_per_sample,
    ...
    # 供 LGBM rerank 成功后二次 type-aware：必须用「第一次 type-aware 之前」的 per-sample 基底，
    # 否则评估仍读 per_sample_results 时不会反映学习排序（RCAbench 走 per-sample 评估）。
    ...
    def _build_union_reranker_pool() -> List[Tuple[int, float]]:
        score_map: Dict[int, float] = {int(pid): float(score) for pid, score in transformed_pod_scores}
        ...
        for rec in stage1_per_sample:
            for pid in (rec.get('candidate_services') or []) if isinstance(rec, dict) else []:
                _add_pid(pid)

        return [(pid, float(score_map.get(pid, 0.0))) for pid in ordered_ids]

    def _build_stage1_pool_stats(pool_ids: List[int]) -> Tuple[Dict[int, int], Dict[int, float], Dict[int, float]]:
    ...
    # Optional data dump for offline learning-to-rank training.
    if reranker_dump_path and stage1_per_sample and groundtruths and reranker_pool:
        try:
            ...
            vote_counts_map, stage1_sum_map, stage1_peak_map = _build_stage1_pool_stats(pool_ids)

            def _gt_pod_set(gt_item) -> Set[int]:
                result: Set[int] = set()
                if gt_item is None:
                    return result
                items = list(gt_item) if isinstance(gt_item, (list, set, tuple)) else [gt_item]
                for it in items:
            ...
            with open(reranker_dump_path, 'w', newline='', encoding='utf-8') as f:
                ...
                for sample_idx, rec in enumerate(stage1_per_sample):
                    rec_scores = rec.get('scores') or []
                    gt_set = _gt_pod_set(groundtruths[sample_idx] if sample_idx < len(groundtruths) else None)
                    for pid in pool_ids:
                        stage1_sample = float(rec_scores[pid]) if 0 <= pid < len(rec_scores) else 0.0
                        ...
                        writer.writerow(
                            {
                                ...
                                'stage1_peak_score': float(stage1_peak_map.get(pid, 0.0)),
                            }
                        )
    ...
    # Optional model-based rerank (LightGBM ranker/classifier).
    if enable_stage2_reranker and reranker_model_path and reranker_pool and stage1_per_sample:
        try:
            ...
            if rows:
                ...
                hybrid_model = isinstance(model, dict) and model.get('scoring') == 'rrf_top1_then_enhanced'
                direct_pack_model = isinstance(model, dict) and model.get('scoring') == 'direct_predict'
                if hybrid_model:
                    if _stage2_prepare_feature_frame is None or _stage2_assign_hybrid_scores is None:
                    ...
                    feature_df, _ = _stage2_prepare_feature_frame(feature_df, 'enhanced')
                    base_cols = list(model.get('base_feature_cols') or STAGE2_RERANKER_BASE_FEATURE_COLS)
                    enhanced_cols = list(model.get('enhanced_feature_cols') or [])
                    missing_cols = [c for c in enhanced_cols if c not in feature_df.columns]
                    if missing_cols:
                        raise ValueError(f'Hybrid reranker feature columns missing during inference: {missing_cols[:8]}')
                    ...
                    if feature_set == 'base':
                        feature_df = pd.DataFrame(rows, columns=list(STAGE2_RERANKER_BASE_FEATURE_COLS))
                    else:
                        if _stage2_prepare_feature_frame is None:
                            raise RuntimeError('Enhanced reranker helper is unavailable.')
                        feature_df = pd.DataFrame(rows, columns=list(STAGE2_RERANKER_BASE_FEATURE_COLS))
                        feature_df.insert(0, 'pod_id', row_pids)
                        feature_df.insert(0, 'sample_idx', row_sample_indices)
                        feature_df.insert(2, 'y', 0)
                        feature_df, _ = _stage2_prepare_feature_frame(feature_df, feature_set)
                    feature_cols = list(model.get('feature_cols') or STAGE2_RERANKER_BASE_FEATURE_COLS)
                    pred = model['model'].predict(feature_df[feature_cols].to_numpy(dtype=np.float32))
                ...
                base_vals = th.tensor([pool_score_map.get(pid, 0.0) for pid in pool_ids], dtype=th.float32)
                blend = float(max(0.0, min(1.0, reranker_blend)))
                # 在池内对 pred 做 min-max，再线性映射到 base 的 [min,max]，与原始 stage2 同量纲混合。
                # 避免「base 与 pred 各自归一化后再混合」把分数全压到 [0,1] 写回 stage2，导致后续
                # per-sample 再次归一时放大热点/真值的相对位移（replace-body 等类型上尤为明显）。
                pred_by_sample_pid: Dict[int, Dict[int, float]] = defaultdict(dict)
                for row_idx, pid in enumerate(row_pids):
                    sample_idx = row_sample_indices[row_idx] if row_idx < len(row_sample_indices) else 0
                ...
                for sample_idx in range(len(stage1_per_sample)):
                    ...
                    pred_vals = th.tensor([sample_pred.get(pid, 0.0) for pid in pool_ids], dtype=th.float32)
                    ...
                    pred_scaled_by_sample.append(
                        {int(pool_ids[j]): float(pred_scaled[j].item()) for j in range(len(pool_ids))}
                    )

                pred_scaled = th.tensor(
                    [
                        float(np.mean([sample_map.get(pid, 0.0) for sample_map in pred_scaled_by_sample]))
                        for pid in pool_ids
                    ],
                    dtype=th.float32,
                )
                ...
                sorted_idx = sorted(range(len(pool_ids)), key=lambda j: float(blended[j]), reverse=True)
                reranked_pool = [(int(pool_ids[j]), float(blended[j])) for j in sorted_idx]
                tail = [(int(pid), float(score)) for pid, score in final_shared_ranking if int(pid) not in set(pool_ids)]
                lgbm_shared_top = (reranked_pool + tail)[:top_k_pods]

                transformed_pre_rerank = list(transformed_pod_scores)
                base_stage2_full = {int(pid): float(sc) for pid, sc in transformed_pre_rerank}
                ...
                reranker_diag.update(
                    {
                        ...
                        'mode': 'hybrid_model' if hybrid_model else ('pack_model' if direct_pack_model else 'model'),
                        'pool_ids': pool_ids,
        ...
        'stage2': {
            ...
            'shared_diagnostics': {
                ...
                'top_shared_breakdown': [
                    {
                        'pod_id': int(pid),
                        'shared_stage1_score': float(stage1_vals[idx]) if idx < len(stage1_vals) else 0.0,
                        'shared_stage2_score': float(stage2_vals[idx]) if idx < len(stage2_vals) else 0.0,
                        'shared_source_score': float(source_vals[idx]) if idx < len(source_vals) else 0.0,
                        'vote_count': int(vote_counts[idx]) if idx < len(vote_counts) else 0,
...
def lag_aware_dual_stage_rca(
    ...
    **kwargs,
    ...
    """Public RCA entrypoint. Dispatches to the single active implementation."""
